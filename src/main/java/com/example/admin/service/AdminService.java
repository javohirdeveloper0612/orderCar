        ...
        for (OrderClientEntity entity : list) {
            myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                    ...
                    InlineButton.keyboardMarkup(InlineButton.rowList(
                            InlineButton.row(InlineButton.button("Завершить заказ ✅", "accept_order#" + entity.getId())),
                            InlineButton.row(InlineButton.button("Адрес, откуда уходит машина \uD83D\uDCCD", "loc1#" + entity.getId())),
                            InlineButton.row(InlineButton.button("Пункт назначения автомобиля \uD83D\uDCCD", "loc2#"+entity.getId()))))));
